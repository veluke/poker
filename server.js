...
io.on('connection', function(socket) {
    ...
    socket.on("create_room", room => {
        ...
        for (var i = 1; i < userRooms.length; i++) {
            var iRoom = userRooms[i];
			socket.leave(iRoom);
			console.log(socket.id + " left " + iRoom);
			var leftRoom = findKey(users,'room',iRoom);
			leftRoom.users = (leftRoom.users -1);
			var updateUser = findKey(turns,"room",iRoom);
			console.log(updateUser.p1 + " | " + updateUser.p2);
			if (updateUser.p1 == socket.id){
			updateUser.p1 = "";}
		else if (updateUser.p2 == socket.id){
			updateUser.p2 = "";}
			console.log(updateUser.p1 + " | " + updateUser.p2);
       }
    ...
    socket.on("join_room", room => {
		...
		let roomUsers = findKey(users,"room",room);
